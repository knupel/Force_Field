...
void force_clear_import() {
  ...
  if(get_files() != null) {
    ...
    get_files().clear();
    warp_force.image_library_clear();
    movie_library_clear();
...
void reset_field(int type, int pattern, int super_type, int resolution) {
  ...
  if(pattern == IMAGE) {
    build_ff(type,pattern,resolution, warp_force.get_image(), get_sorting_channel_ff_2D());
  } else {
...
void diaporama(int type, int tempo_diaporama) {
  if(warp_force.library_size() > 1 && diaporama_is) {
    if(frameCount%tempo_diaporama == 0) {
      tempo_diaporama = int(random(240,1200));
      if(type == r.CHAOS) {
        which_img = floor(random(warp_force.library_size()));
        if(which_img >= warp_force.library_size()) {
...
void check_current_img_size_against_display() {
  ivec2 display = get_display_size();
  if(warp_force.get_image().width > display.x || warp_force.get_image().height > display.y) {
    ivec2 new_size_img = def_window_size(warp_force.get_image().width, warp_force.get_image().height);
    warp_force.get_image().resize(new_size_img.x,new_size_img.y);
  }
}
